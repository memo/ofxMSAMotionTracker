...
//		gui.loadFromXML();
	...
	void MotionTracker::allocateTextures(ofPixelsRef inputImage) {
		...
		if(inputImage.getNumChannels() > 1) {
			captureImage = new ofxCvColorImage();
			if(settings.scaleDown>1) resizedImage = new ofxCvColorImage();
			else resizedImage = captureImage;
			maskedImage = new ofxCvGrayscaleImage();
		} else {
			captureImage = new ofxCvGrayscaleImage();
			if(settings.scaleDown>1) resizedImage = new ofxCvGrayscaleImage();
			else resizedImage = captureImage;
			maskedImage = new ofxCvGrayscaleImage();
		}
	...
	void MotionTracker::reset() {
		...
		if(captureImage) captureImage->set(0);
		if(resizedImage) resizedImage->set(0);
		if(maskedImage) maskedImage->set(0);
		cleanPlate.set(0);
		processedCleanPlate.set(0);
		currentGreyImage.set(0);
		processedGreyImage.set(0);
		previousGreyImage.set(0);
		currentDiff.set(0);
		accDiff.set(0);
		opFlowInput1.set(0);
		opFlowInput2.set(0);
...
//        if(!isReady()) return;
		...
		if(inputImage.getNumChannels() > 1) {
			currentGreyImage = *(ofxCvColorImage*)resizedImage;
		} else {
			currentGreyImage = *(ofxCvGrayscaleImage*)resizedImage;
		}
		...
		if(frameCounter > 2) {		// dont do anything until we have enough in history
			...
			/*			
			 ...
			 imgCameraGrab.saveImage("camgrabs/camgrab_" + ofToString(ofGetYear(), 4) + ofToString(ofGetMonth(), 4) + ofToString(ofGetDay(), 4) + "_" + ofToString(ofGetFrameNum()) + ".tif");
